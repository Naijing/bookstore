...
bookListModule.controller('BookListCtrl', function($scope, $http, $state, $stateParams) {
    ...
    $scope.gridOptions = {
        data: 'books',
        rowTemplate: '<div style="height: 100%"><div ng-style="{ \'cursor\': row.cursor }" ng-repeat="col in renderedColumns" ng-class="col.colIndex()" class="ngCell hello ">' +
...
bookDetailModule.controller('BookDetailCtrl', function($scope, $http, $state, $stateParams) {
    ...
    $http.get('../data/books0.json')
                    .success(function(data) {
                        $scope.getbookbyid(data, $stateParams.bookId);
                    });
//console.log($scope.data);
                    //console.log(angular.isArray($scope.data));

    //angular.forEach($scope.data, function(value, key) {
      /*if(value.bookId == $stateParams.bookId){
        console.log(value.name);
      }*/
     // console.log('hello');
    ///});

    $scope.getbookbyid = function(books, id){

        angular.forEach(books, function(value, key) {
      if(value.bookId == id){
        console.log(value);
        $scope.thisbook = value;
        return false;
      }
    });
